...
| 탭 | 하는 일 |
|---|---|
| **대화** | 채팅형 뷰어. 채널·날짜 범위, 빠른 칩, 본문 검색(하이라이트), 원문으로 점프, **정리** 버튼 |
| **묻기** | 화면 전체를 쓰는 채팅. 고른 채널·기간의 대화를 자료로 놓고 묻고 시킨다. 답은 타이핑되듯 흐르고, 자주 쓰는 건 버튼으로 — 요약 · 아이디어 · 결정/미결 · 참여 · 할 일 |
| **리더보드** | 발언 수 · 활동일 · 채널 수, 기간·채널 필터 |
| **관리 › 수집 채널** | 채널 켜고 끄기, 과거 대화 가져오기 — *서버 관리 권한자만* |
| **관리 › 일일 결산** | 결산 채널·시각·모델, 수동 실행, 원문 보존 기간 — *서버 관리 권한자만* |
| **설정 › 계정 연결** | 에이전트 CLI 계정, 디스코드 연동 |
| **설정 › MCP** | 기본 제공 MCP 원클릭, 온디맨드 주소 입력 |
| **설정 › 내 설정** | 내 메시지 수집 거부 |

사이드바는 **접을 수 있고**(브랜드 옆 화살표), 상위 탭 **관리·설정**을 누르면
아래로 세부 탭이 펼쳐진다. 접힌 상태와 펼친 그룹은 브라우저에 기억된다.
